...
[System.Serializable]
public class Phi3Config : PretrainedConfig {
...
public class Phi3 : ModelForCausalLM<Phi3Config> {
	public Phi3(TensorNN nn, TextAsset configJson): base(nn, configJson) {}
	public override (Texture, Texture) ForCausalLM(Texture input_ids) => Phi3ForCausalLM(input_ids);

	void Phi3Attention(ref Texture hidden_states, Texture input_ids, string path, int layer_id) {
	...
	void Phi3MLP(ref Texture hidden_states, string path) {
	...
	void Phi3DecoderLayer(ref Texture hidden_states, Texture input_ids, string path, int layer_id) {
		var attn_states = nn.GroupNorm(hidden_states, state_dict[$"{path}.input_layernorm.weight"], null, config.rms_norm_eps, rmsNorm:true);
		Phi3Attention(ref attn_states, input_ids, path:$"{path}.self_attn", layer_id:layer_id);
	...
	(Texture, Texture) Phi3ForCausalLM(Texture input_ids) {
		var hidden_states = Phi3Model(input_ids, path:"model");
