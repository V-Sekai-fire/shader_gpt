...
public class Phi : GPTBase {
	...
	Texture PhiModel(Texture input_ids, string path) {
		var hidden_states = nn.IndexSelect(parameters[$"{path}.embed_tokens.weight.T"], (input_ids, 0), inputT:true);
	...
	(Texture, Texture) PhiForCausalLM(Texture input_ids) {
		...
		var lm_logits = nn.Linear(hidden_states, parameters["lm_head.weight.T"], parameters["lm_head.bias"], weightT:true);
